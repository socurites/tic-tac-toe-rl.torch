...
cmd:option('-epoch', 10000, 'The number of games we want the system to run for.')
...
local nbStates = gridSize * gridSize * 2
...
function checkWinState(canvas, stone)
    -- horizontal
    for i = 1, 3 do
        if ( canvas[1][i][1] == stone and canvas[1][i][2] == stone and canvas[1][i][3] == stone ) then
            return true
        end
    end
    -- vertical
    for i = 1, 3 do
        if ( canvas[1][1][i] == stone and canvas[1][2][i] == stone and canvas[1][3][i] == stone ) then
            return true
        end
    end
    -- diagonal
    if ( canvas[1][1][1] == stone and canvas[1][2][2] == stone and canvas[1][3][3] == stone ) then
        return true
    end
    if ( canvas[1][1][3] == stone and canvas[1][2][2] == stone and canvas[1][3][1] == stone ) then
        return true
    end
    return false
end
...
function isMarkable(canvas, coord)
    if ( currentState[2][coord[1]][coord[2]] == 0 ) then
        return true
    end
    return false
end
...
for i = 1, epoch do
    ...
    local canvas = torch.Tensor(2, gridSize, gridSize):zero()
    ...
    while (gameOver ~= true) do
        ...
        -- Update Enviroiment
        currentState[1][coordX[1]][coordX[2]] = -1
        currentState[2][coordX[1]][coordX[2]] = 1
        -- check state by User
        ...
        elseif ( isAllMarked(currentState) ) then
            ...
            nextState[1][coordY[1]][coordY[2]] = 1
            nextState[2][coordY[1]][coordY[2]] = 1
